...
def aggregate_metadata(database, outfile, **kwargs):
    exclude_projects = (
        kwargs["exclude_projects"] if "exclude_projects" in kwargs else []
    )
    ...
    for r in cur.execute(
        "SELECT Expedition, Country, State_Province, Location, PI FROM boreholes"
    ):
        [e, c, s, l, p] = r
        expeditions.add(e)
    ...
    query_statment = (
        "SELECT " + ", ".join(query_columns) + " FROM projects ORDER BY Expedition"
    )
    ...
    with open(outfile, "w", encoding="utf-8-sig") as f:
        ...
        for e in sorted(expeditions - set(exclude_projects)):
            ...
            try:
                aggregated_line = (
                    [aggregated_line[0]]
                    + [project_metadata[e][0]]
                    + aggregated_line[1:]
                    + list(project_metadata[e][1:-1])
                )
            except KeyError:
                ...
                print(
                    f"WARNING: Project {e} is not in the projects table in {database}.",
                    flush=True,
                )

            if e in debug_projects:
                print(
                    f"Full, aggregated data for writing:\n{aggregated_line}\n",
                    flush=True,
                )
...
def export_project_location_data(database, outfile, **kwargs):
    ...
    with open(outfile, "w", encoding="utf-8-sig") as f:
        ...
        query_statment = (
            "SELECT "
            + ", ".join(query_columns)
            + " FROM boreholes ORDER BY Expedition, Location, Original_ID"
        )
...
def main():
    ...
    parser = GooeyParser(
        description="Aggregate fields from the CSDCO database by Expedition for Drupal import."
    )
    ...
    if not os.path.isfile(args.database_file):
        print(
            f"ERROR: database file '{args.database_file}' does not exist.", flush=True
        )
        exit(1)

    if not os.path.isdir(args.output_directory):
        print(
            f"ERROR: output folder '{args.output_directory}' does not exist.",
            flush=True,
        )
        exit(1)
    ...
    print(
        f"Completed in {round(timeit.default_timer()-start_time,2)} seconds.",
        flush=True,
    )
